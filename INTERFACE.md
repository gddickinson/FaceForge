...
live in `docs/sex_morph.md`, and the fit of the skeleton into the body-surface
mesh in `docs/skeleton_fit.md`.
...
├── docs/                 headless_cli.md, exercise_animation.md, sex_morph.md, skeleton_fit.md, exercises.md (generated), research/
...
| `foot_level.py` | `FootLevelLock`: the ground lock translates the whole body, which is one correction for two feet. The donor's legs are not identical -- the shanks differ by about seven degrees -- so equal joint angles land the feet at different heights, 6.8 units apart at the bottom of a squat. This brings the higher leg down with its knee and hip, one equation in two unknowns solved for the least costly step (`JOINT_COST` makes the hip four times the knee), carrying the ankle so the sole stays flat (`ANKLE_PER`, the knee adding and the hip subtracting), and puts the pose back untouched if it cannot halve the gap. |
| `ground_contact.py` | Keep the feet (or hands) on the floor while the joints move. The anchor is the **lowest pivot of the whole extremity**, every toe or finger segment included: the ankle and the wrist are the joints at the *top* of a foot and a hand, and anchoring by them let a push-up bury its hands 17 units in the mat while the lock reported them exactly on it. Hands anchored to a point (a bar) are still measured at the closed-finger ring, because the lowest digit pivot there would hang the body off whichever fingertip curled furthest under. The foot target is lowered by `sole_clearance` -- how far the body's lowest point rests above the floor -- because a pivot is inside the bone. |
...
| `fit_regions.py` | The skeleton as a tree of 23 regions for the fit: which node belongs to which region, each region's anchor joint, and the rule that composes rotations down the chain the way a pose does, so turning the humerus carries the whole arm. `SKIP_SUBTREES` holds back the surface the fit aims at, the mammary lens cut from it, and every soft tissue with its own deformer; `DIGIT_REGIONS` names the fingers and toes, which hang beside other parts of the body and so are never evidence for which part a point belongs to. |
...
| `skeleton_field.py` | Turns a skeleton change into a smooth spatial warp: joint displacements as a thin-plate spline (`displacement_warp`), sampled on a lattice (`sampled_warp`). The lattice evaluates only the nodes within `margin` of a control point -- a standing body fills less than a third of its own bounding box -- and a cell whose corners were not all evaluated falls back to the exact warp. |
...
| `exercise.py` | Exercise demonstrations: the app-side glue around :class:`ExerciseRuntime`. `on_export_obj` writes the current frame and opens the viewer: synchronous on purpose, because the exporter walks the live scene graph and the frame loop rewrites every world matrix in place sixty times a second. It hides `scene_env_root` for the duration -- otherwise the export is the building, and the file's bounds come out as the room rather than the athlete. |
...
| `equipment.py` | Procedural gym equipment, built from the scene's own primitives; the treadmill's belt top sits at the sole height the ground lock produces, and a battle rope turns each segment onto its own tangent (offset cylinders read as a staircase). Sizes that were measured against the body rather than chosen: the dip station's 81-unit width (the grip lands at x +-40.4), the mat's 120 (the floor work reaches z +-57), a cable handle's `length` (a two-handed bar has to span the grip), and the rope drops -- a skipping rope's arc has to reach the floor, a battle rope's far end has to be anchored to it. `make_wall` is the piece of wall a wall sit and a standing calf stretch actually touch. A mat's top face **is** the floor plane (the ground lock anchors the body to y = 0 and every static height is measured from there, so a slab laid on the floor put the athlete 1.5 units inside their own mat); a tilted bench hinges at `incline_pivot_x`, under the hip the lifter pitches about, because two rigid bodies turned by the same angle about centres 41 apart come apart by 41 x sin(theta) -- 18.1 of daylight under a 30 deg incline, 15.1 of pad through a 20 deg decline. |
| `equipment_rig.py` | Keep hand-held equipment in the hands, every frame: the bar's axis passes through the ring of closed finger joints (`grip_point`). `attach="knees"` is the same geometry one storey down, for a loop round the legs -- the band walk shipped no band at all while hands and the room were the only attachment points. `attach="floor"` lays a covering down ONCE under the body's low-lying pivots (`_settle_on_floor`): a mat placed at the origin like the rest of the static kit left 21 exercises with most of the athlete off it, a push-up and a downward dog at 0%. |
| `grip_lock.py` | `GripWidthLock`: hands holding a bar stop sliding along it -- either anchored to one in the room (a pull-up) or holding a rigid two-handed implement (`EquipmentSpec.attach == "hands"`, a barbell), the case a bench press is and which used to let each hand slide 13.8 units a rep; per frame, shoulder abduction is solved (2x2 finite-difference Newton) so each hand's offset from the trunk holds its calibrated value.  Where it cannot solve a frame -- a singular Jacobian, or an error past `_MAX_ERROR` -- it gives up rather than divide by nearly nothing, and `_carry` then holds the last abduction it actually *solved*: leaving the authored pose there sprang the bench press's hands from 110 units apart to 161 and back, and its span ranged 52.0 units over a rep against 16.6 carried.  Its one lever runs out at the finish of a PULL, where abduction is weak (6-14 units of hand travel per unit) or points the wrong way (at the bottom of a pulldown it *narrows* the grip) -- the rows and the pulldown are that limit, not a bug. |
| `model.py` | The data model for an exercise demonstration; `EquipmentSpec.hang` overrides how far below the hands a held item's origin sits (a goblet-held kettlebell, a front-racked bar); `EquipmentSpec.attach` also takes `"knees"` and `"floor"`. |
...
| `pose_library.py` | Pose authoring for exercises: degrees in, normalised BodyState DOFs out.  `flat_foot_ankle` keeps a sole flat; `toes_on_floor` bends the toes of a foot whose heel has left the floor (a calf raise, a jump take-off, the back foot of a lunge) and `toes_tucked` those of a foot tucked under a prone body (a plank, a push-up).  `DORSUM_DOWN` is the back ankle past which a split stance is a half-kneel on the foot's *instep*, where the toes lie flat and bending them back 75 degrees drives the tips through the floor.  Without them the foot is one rigid wedge and the model balances on the point of its longest toe -- and drives it through the floor. |
...
| `olympic.py` | The competition lifts and their assistance work: snatch (squat and power), clean and jerk, hang power clean, push and split jerk, overhead squat, clean and snatch pulls.  All filmed three-quarter, because a bar along X is invisible end-on from the `side` camera. |
| `yoga.py` | Eight held asanas, entered / held / released: chair, warrior II, triangle, tree, high lunge, downward and upward dog, cat-cow. The prime mover is whatever holds the shape, so most of the work is isometric. A visible trunk lean is a wrapper `roll`/`pitch` about the hips, never a spine DOF: measured, `spine_flex`/`spine_lat_bend`/`spine_rotation` at full range move the shoulder, hip, wrist and knee pivots by 0.0 units, so triangle pose rendered as a man standing upright with his arms out. Cobra proper is absent for the same reason -- its whole shape is spinal extension with the pelvis down, and pitching about the hips takes the legs through the mat (feet at -88). |
...
| `gl_widget.py` | PySide6 QOpenGLWidget subclass bridging Qt and OpenGL rendering; `initializeGL` logs the context it was actually handed (`_context_summary`, which cannot raise -- it runs inside the `try` it only describes). |
...
| `renderer.py` | Main OpenGL renderer -- traverses the scene graph and issues draw calls. `init_gl` calls `drain_errors` first: a context arrives with errors already queued that are the toolkit's, not ours (macOS leaves a `GL_INVALID_ENUM` behind in every window shape measured), and PyOpenGL checks `glGetError` *after* each call -- so the flag was charged to `glClearColor`, which cannot raise it. `init_gl` then died on its first line, the shaders were never compiled, `render` returned early forever on `_initialised`, and the window stayed one flat colour in silence. In `init_gl` rather than its callers because the widget, the headless `Session` and the capture tools all need it. |
...
| `procedural_geometry.py` | Procedural mesh builders for scene environment objects. `make_cylinder`'s caps indexed into the side wall (`len(positions) // 3` on a list of 3-tuples), so every cylinder in the scene -- barbell plates, dumbbell heads, uprights, the rower flywheel -- drew a black pinwheel where its ends should be. |
...
| `obj_viewer.py` | A standalone window that displays ONE OBJ file, in its own process (`launch`) so a slow load or a crash cannot take the application with it. `read_obj_fast` is a vectorised reader for the files this is given: `loaders.obj_parser` walks a line at a time, which is right for the app's assets and hopeless for a posed scene -- 3.46 M vertices takes it ~105 s against 13. The file is turned Y-up (the gym world) into Z-up (the body frame, and what `OrbitControls` measures its polar angle from), or the model arrives lying on its side. |
...
| `exercise_tab.py` | Exercise tab: pick a demonstration, read the technique, watch the muscles work. "Save OBJ && view" writes the frame **on screen** and opens `obj_viewer` on it, so the layer toggles decide what the file contains -- with the skeleton showing, a squat is 3.5 M vertices and 556 MB, which is why the vertex count and the size are reported back. |
...
| `render_exercise_views.py` | One sheet per exercise: three phases x three cameras (front / side / three-quarter) on a world-space grid drawn from the camera's own view-projection -- 50-unit floor lattice, height rule every 25 through the body's measuring plane, joint-pivot dots. The camera is framed **once per exercise** from every phase at once, because re-framing per phase makes the frames incomparable: a calf raise that lifts the body 12 units looks identical in two frames the camera has silently zoomed to match. `pick_phases` takes the first phase, the one furthest from it and the last -- evenly spaced *indices* skip index 1, which on a four-phase exercise is where the movement is |
| `audit_exercise_placement.py` | Every joint pivot's height per phase: what is through the floor and what is floating above it (`THROUGH`, `FLOATING`) |
| `audit_exercise_motion.py` | The complementary question to placement, which the ground lock pins by construction: does the exercise actually *move*? Per-exercise largest pivot excursion, heel lift where the name implies a jump, and the lowest world point of each item's own geometry -- which is how the loaded bar was found hanging 17 to 52 units above the floor on every lift that starts there |
| `audit_room_fit.py` | The ceiling and the walls, which nothing else checks: the body (real cranium mesh, not a proxy) and every equipment vertex against the gym's own box. The gym is the **studio** room, `STUDIO_HEIGHT` 300 / `STUDIO_WIDTH` 500 / `STUDIO_DEPTH` 400 -- `ROOM_HEIGHT` (250) is the clinical scene, and measuring the gym against it overstates every breach by 50 and makes every overhead lockout look like one |
| `audit_equipment_contact.py` | The opposite question: is the body actually *touching* its equipment? Per equipment PART, not per item -- a pull-up frame is mostly air, and one box round the whole of it called every pull-up a miss.  `oriented_parts` gives each part its OWN box, axes read off its world matrix: an AABB round a rotated part is mostly air, and a 150 x 6 bench pad tilted 30 degrees has an 80-unit-tall AABB, which is why "the clavicle is 15.3 inside the bench pad" could never be acted on |
| `audit_equipment_clash.py` | And the opposite of that: is the equipment inside the body? Capsules per bone segment plus a skull sphere against each equipment part's box, sampled *through* each phase rather than at its keyframes, because a muscle-up's bar crosses the chest between two frames that are both clear. Support surfaces (a pad, a seat, a box) are counted separately, and `--support` prints them: "resting on the pad" and "the collarbone is 15 through a 6-unit pad" are not the same reading, and suppressing both hid the second for a whole catalogue |
| `audit_foot_contact.py` | The feet: which pivot is lowest, how far the heel is above it, whether the toe tips are below the ball (a rigid wedge standing on its point), and whether anything has gone through the floor or through the box it is standing on |
