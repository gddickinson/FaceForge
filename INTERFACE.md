...
| `pose_library.py` | Pose authoring for exercises: degrees in, normalised BodyState DOFs out.  `flat_foot_ankle` keeps a sole flat; `toes_on_floor` bends the toes of a foot whose heel has left the floor (a calf raise, a jump take-off, the back foot of a lunge) and `toes_tucked` those of a foot tucked under a prone body (a plank, a push-up).  Without them the foot is one rigid wedge and the model balances on the point of its longest toe -- and drives it through the floor. |
...
| `audit_equipment_contact.py` | The opposite question: is the body actually *touching* its equipment? Per equipment PART, not per item -- a pull-up frame is mostly air, and one box round the whole of it called every pull-up a miss |
